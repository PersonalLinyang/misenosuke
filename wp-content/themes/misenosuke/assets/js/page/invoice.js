const resizePreview = function() {
  var area_width = $('.invoice-preview-area')[0].offsetWidth;
  var section_width = $('.invoice-preview-section').width();

  if (area_width > section_width) {
    var scale = section_width / area_width;
    $('.invoice-preview-area').css('transform', 'scale(' + scale + ')');
  } else {
    $('.invoice-preview-area').css('transform', 'none');
  }
}

$(document).ready(function() {
  // ダウンロートボタンをクリック
  $('.invoice-download').click(async function() {
  ...
  // 印刷ボタンをクリック
  $('.invoice-print').click(async function() {
    // HTMLからキャンバスを作成
    const content = document.getElementById('invoice-preview');
    const canvas = await html2canvas(content, { scale: 3 });
    ...
    print_window.document.write('<html><head><title>' + ucwords(translations.print_invoice_document) + '</title>');
